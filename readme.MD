...
A standalone VR/MR application for Gen1.
...
The **VR** options row (OFF / DIORAMA / DIORAMA-MR / 1ST PERSON, off by
default) drives a PCVR headset through OpenXR on Windows — SteamVR,
Oculus or WMR — or the Quest 3 standalone. Three modes, and **the left
stick's click steps round them**.

**1ST PERSON** puts the headset in the player's *head* at life size, and
holds the VOXEL ladder on its 1ST rung for as long as the mode is live —
so the walk, the sprites and the right stick's turn are the 1ST rung's.
(A boom that seats its wearer three cells behind their own body is a
reliable way to make people ill, which is why there is no 3RD here.)

**The world bakes once, up front.** The first run after an install bakes
every map's geometry to disk behind a plate that says so — now as soon as
you leave the launcher, before the main menu, rather than on your first
step into the world. **B** skips it and the rest finishes quietly behind
play.

**The Pokédex in your left hand is the game's UI, in every mode.** Menus,
dialogs and boxes light up on the device's screen — over the tabletop, in
the diorama, inside the world and through every fight — drawn from the
engine's own UI layer at a clean integer scale, never as a video of the
flat window with the world rendered twice behind it. During a battle the
screen shows the classic 160×144 battle frame — both HUDs and the text
box on its own paper — so the fight reads exactly as the Game Boy laid it
out, in your hand. No tracked controller (or no world, at the title
screen) falls back to a floating panel; everywhere else the panel stays
retired.
...
- **A staged fight** ignores both and cuts a vertical pillar about the
  ...
  map as a floating disc — right where the model is, but at its own size
  and facing rather than the table's: it lies on the table exactly as its
  arena lies on the map, and both grips turn or resize the fight itself,
  which is remembered for the next one. In **DIORAMA-MR** the **MR FIGHT** row can set the fight
  down on your room's real floor instead, at three-foot scale on its own
  disc of ground (or the battle discs, on a disc rung); TABLE is the
  default.
...
- **The ladder is held on an orbit rung** while the mode runs — there is
  no 2D diorama and no first-person one. **V-CURVE** is the right stick's
  click (one rung per click, round the ladder), the "7" key, and its
  OPTIONS row.
